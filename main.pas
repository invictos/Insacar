...
procedure scoreenr( infoPartie: T_GAMEPLAY);
                                 //PITIE NE REGARDEZ PAS CETTE MERDE
{var fic : Text;
    dechet : string;
    recupbrut: array [0..39] of String;
    recupint: array[0..2,0..39] of integer ;
    i,j,tours,taille,fin,check,k : integer;
    a,b,c,d : char;
    sup : array [0..1] of integer;
    ok,verouille:  boolean;
begin
        ...
        begin
                if (i mod 3 = 0) then
                        begin
                        readln(fic,recupbrut[taille]);
                        taille:=taille+1;
                        end
                        else readln(fic,dechet);
        ...
        begin
                ...
                read(recupbrut[i],c);
                read(recupbrut[i],d);
                recupint[k][i]:=(StrtoInt(a)*100)+(StrtoInt(b)*10)+StrtoInt(c);
                end;
        end;
        i:=0;
        tours:=0;
        ...
        repeat
                ...
                if ((recupint[0][i] = (Round(infoPartie.hud.temps_tour^.valeur) div 60000))and not(verouille)) then
                begin
                        sup[0]:=i;
                        verouille := True ;
                end;

                if recupint[0][i]> (Round(infoPartie.hud.temps_tour^.valeur) div 60000) then
                begin
                        j:=i-1;
                        if j = sup[0]-1then
                        begin
                        ...
                        begin
                                verouille :=False;
                                check:=sup[0];
                                while(not(ok) and  (check <= j)) do
                                        begin
                                                if ((recupint[1][check]= (StrtoInt(infoPartie.hud.temps_tour^.valeur) div 60000))and not(verouille)) then
                                                begin
                                                        sup[1]:=check;
                                                        verouille := True ;
                                                end;

                                                if  recupint[1][check] > ((StrtoInt(infoPartie.hud.temps_tour^.valeur) mod 60000) div 1000)then
                                                begin
                                                        j:=check-1;
                                                        ...
                                                        begin
                                                                ...
                                                                while((not(ok)) and  (check <= j)) do
                                                                begin
                                                                        if (recupint[2][check]>(StrtoInt(infoPartie.hud.temps_tour^.valeur) mod 1000)) then
                                                                        begin
                                                                        fin:=check-1;
                                                                        ok:=true;
                                                                        end;
...
begin
	...
	fenetre.enfants.t[fenetre.enfants.taille-1]^.etat.x := 350;
	...
	begin
		...
		readln(fichier,fenetre.enfants.t[fenetre.enfants.taille-1]^.enfants.t[fenetre.enfants.t[fenetre.enfants.taille-1]^.enfants.taille-1]^.valeur);
	...
	fenetre.enfants.t[fenetre.enfants.taille-4]^.surface := IMG_Load('tuto/space.png');
	fenetre.enfants.t[fenetre.enfants.taille-4]^.etat.x := fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.x+375;
	fenetre.enfants.t[fenetre.enfants.taille-4]^.etat.y := fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.y+180;

	fenetre.enfants.t[fenetre.enfants.taille-3]^.surface := IMG_Load('tuto/ctrl.png');
	...
	fenetre.enfants.t[fenetre.enfants.taille-3]^.etat.y := fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.y+230;
        ...
        fenetre.enfants.t[fenetre.enfants.taille-2]^.surface :=IMG_Load('tuto/flechegauche.png');
        fenetre.enfants.t[fenetre.enfants.taille-2]^.etat.x:= fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.x+300;
        fenetre.enfants.t[fenetre.enfants.taille-2]^.etat.y:= fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.y+300;

        fenetre.enfants.t[fenetre.enfants.taille-1]^.surface := IMG_Load('tuto/flechedroite.png');
        fenetre.enfants.t[fenetre.enfants.taille-1]^.etat.x:= fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.x+400;
        fenetre.enfants.t[fenetre.enfants.taille-1]^.etat.y:= fenetre.enfants.t[fenetre.enfants.taille-5]^.etat.y+300;
...
var	event_sdl: TSDL_Event;
	...
	actif : Boolean;
        ...
        recup : array [0..2,0..17] of string;
begin
	...
	fenetre.enfants.taille:=0;
        ...
        arial := TTF_Openfont('arial.ttf',20);
	Assign(fichier, 'score/scorereg.txt');
	reset(fichier);
	...
	begin
		...
		fenetre.enfants.t[fenetre.enfants.taille-1]^.etat.y := 80 ;
                fenetre.enfants.t[fenetre.enfants.taille-1]^.surface := SDL_DisplayFormat(IMG_Load('score/grey_panel.png'));
		fenetre.enfants.t[fenetre.enfants.taille-1]^.typeE := image;
		max:=0;

		//interieur des scores tableau

		while (not(Eof(fichier)) and (max<17)) do
		begin
			for i:=0 to 2 do
			begin
	...
	fenetre.enfants.t[fenetre.enfants.taille-1]^.etat.y := 600;
	fenetre.enfants.t[fenetre.enfants.taille-1]^.surface :=IMG_Load('jeu_menu/back-button.png');
        fenetre.enfants.t[fenetre.enfants.taille-1]^.typeE := image;
